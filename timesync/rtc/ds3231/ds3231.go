// Adapted from https://github.com/tinygo-org/drivers
...
func (d *Device) Status() (byte, error) {
	data := []byte{0}
	err := d.ReadRegister(REG_STATUS, data)
	if err != nil {
		return 0, err
	}
	return data[0], nil
...
func (d *Device) Control() (byte, error) {
	data := []byte{0}
	err := d.ReadRegister(REG_CONTROL, data)
	if err != nil {
		return 0, err
	}
	return data[0], nil
...
func (d *Device) SetRunning(isRunning bool) error {
	...
	err = d.WriteRegister(REG_CONTROL, data)
	if err != nil {
		return err
	}
	return nil
...
func (d *Device) SetTime(dt time.Time) error {
	...
	data[0] &^= 1 << OSF
	err = d.WriteRegister(REG_STATUS, data)
	if err != nil {
		return err
	}
	...
	err = d.WriteRegister(REG_TIMEDATE, data)
	if err != nil {
		return err
	}
...
func (d *Device) ReadTime() (dt time.Time, err error) {
	data := make([]uint8, 7)
	err = d.ReadRegister(REG_TIMEDATE, data)
	if err != nil {
		return
	}
...
func (d *Device) ReadTemperature() (int32, error) {
	data := make([]uint8, 2)
	err := d.ReadRegister(REG_TEMP, data)
	if err != nil {
		return 0, err
	}
